...
export function apply(ctx) {
  ...
  // ==================== 北京时间（UTC+8）时段计算 ====================
  // 规则：高峰时段仅周一至周五生效；周六/周日全天为低谷。全部按北京自然日判定，
  // 周界跨零点正确处理（如周五 23:00 仍属周五，周六 00:00 起即低谷）。
  const HOUR = 3600 * 1000
  const MIN = 60 * 1000
  const beijingMinutes = (date) => (date.getUTCHours() * 60 + date.getUTCMinutes() + 480) % 1440
  const beijingDayOfWeek = (date) => new Date(date.getTime() + 8 * HOUR).getUTCDay() // 0=周日 … 6=周六
  const isWeekend = (date) => {
    const d = beijingDayOfWeek(date)
    return d === 0 || d === 6
  }
  const parseMin = (s) => {
    const p = s.split(':')
  ...
  // 时段判定（基于具体北京时刻）：周末全天低谷；工作日按窗口判定。
  const phaseAt = (date) => {
    if (isWeekend(date)) return 'low' // 周六/周日：全天低谷
    const m = beijingMinutes(date)
    return windowsOf().some((w) => inWindow(m, w)) ? 'peak' : 'low'
  }
  // 距下一阶段切换的分钟数（跨天扫描，正确处理周末→周一、窗口起止等切换点）。
  const nextTransitionMinutes = (now) => {
    const nowMs = now.getTime()
    const nowPhase = phaseAt(now)
    // 某北京自然日的 00:00 对应 UTC 时刻
    const beijingDayStartMs = (ms) => {
      const d = new Date(ms + 8 * HOUR)
      return Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()) - 8 * HOUR
    }
    const buf = configRef.bufferMinutes || 0
    const candidates = []
    for (let day = 0; day <= 8; day++) {
      const dayStart = beijingDayStartMs(nowMs) + day * 24 * HOUR
      candidates.push(dayStart) // 每日 00:00 边界
      for (const w of windowsOf()) {
        candidates.push(dayStart + ((w.start - buf + 1440) % 1440) * MIN)
        candidates.push(dayStart + ((w.end + buf) % 1440) * MIN)
      }
    }
    const future = [...new Set(candidates)]
      .map((t) => t - nowMs)
      .filter((d) => d > 1000) // 至少 1 秒后
      .sort((a, b) => a - b)
    for (const d of future) {
      if (phaseAt(new Date(nowMs + d)) !== nowPhase) return d / MIN
    }
    return 1440 // 兜底
  }
  ...
  const sessionExempt = new Map() // sessionId -> true（"放行此次会话"：本高峰周期内该会话不再拦截，回落低谷时清空）
  ...
  const snapshot = (now) => {
    ...
    return {
      ...
      activeCount,
      nextTransitionMinutes: nextTransitionMinutes(now),
      banner,
  ...
  ctx.on('agent/request', async (payload, next) => {
    ...
    if (sessionExempt.has(sessionId)) return config // 模态框"放行此次会话"：本高峰周期内该会话直接放行
    const now = new Date()
    if (phaseAt(now) !== 'peak') return config // 低谷：直接放行（发起瞬间快照判定）
  ...
  const tickId = ctx.setInterval(() => {
    ...
    if (peak) {
      ...
      sessionExempt.clear() // 回落低谷：一并重置"放行此次会话"豁免，下次高峰需重新选择
    }
      ...
      try {
        disposers.push(webServer.register({
          ...
          handler: async (req, res) => {
            ...
            if (body?.decision === 'bypass-session') {
              // "放行此次会话"：本次请求继续，并豁免该会话直到本次高峰周期结束
              sessionExempt.set(entry.sessionId, true)
              entry.settle('continue')
            } else {
